...
void Random::setSeedIndex(int index)
{
    if (index == -1)
    {
        std::srand(std::time(0));
    ...
    std::srand(randomizerSeeds[index]);
}
...
int Random::iuniform(int min, int max)
{
    if (min == max) {
        return min;
    }
