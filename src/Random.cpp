...
void Random::setSeedIndex(unsigned int seedIndex)
{
    ...
    srand(randomizerSeeds[seedIndex]);
}
...
bool Random::getUniformBool()
{
    return ((rand() % 2) == 0);
}

float Random::getUniformFloat(float start, float end)
{
    float span = end - start;
    return (((float)rand()) / ((float)RAND_MAX) * span) + start;
}
